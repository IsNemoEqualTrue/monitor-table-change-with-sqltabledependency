...
{
    public class TeardownTestSqlserver6Model
    {
    ...
    [TestClass]
    public class TeardownTestSqlserver6
    {
        private static readonly string ConnectionString = ConfigurationManager.ConnectionStrings["SqlServer2008 Test_User"].ConnectionString;
        private static readonly string TableName = "TeardownTestSqlserver6Model";

        private static int _counter;
        private static List<TeardownTestSqlserver6Model> _insertedValues = new List<TeardownTestSqlserver6Model>();
        private static List<TeardownTestSqlserver6Model> _notifiedValues = new List<TeardownTestSqlserver6Model>();
        ...
        public void TeardownTests()
        {
            ...
            var mapper = new ModelToTableMapper<TeardownTestSqlserver6Model>();
            mapper.AddMapping(c => c.Description, "Long Description");

            var tableDependency = new SqlTableDependency<TeardownTestSqlserver6Model>(ConnectionString, mapper: mapper, teardown: false);
            ...
            Assert.AreEqual(_notifiedValues.Count, 6);
            ...
            tableDependency = new SqlTableDependency<TeardownTestSqlserver6Model>(ConnectionString, mapper: mapper, teardown: true, namingForObjectsAlreadyExisting: dataBaseObjectsNamingConvention);
            tableDependency.Start();
        ...
        private static void TableDependency_Changed(object sender, RecordChangedEventArgs<TeardownTestSqlserver6Model> e)
        {
            switch (e.ChangeType)
            {
                case ChangeType.Insert:
                    _notifiedValues.Add(new TeardownTestSqlserver6Model() { Id = e.Entity.Id, Name = e.Entity.Name, Description = e.Entity.Description });
                    break;
            }
        }
        ...
        private static void ModifyTableContent()
        {
            using (var sqlConnection = new SqlConnection(ConnectionString))
            {
                ...
                using (var sqlCommand = sqlConnection.CreateCommand())
                {
                    for (var i = 0; i < 3; i++)
                    {
                        _insertedValues.Add(new TeardownTestSqlserver6Model { Id = _counter, Name = _counter.ToString(), Description = _counter.ToString() });
